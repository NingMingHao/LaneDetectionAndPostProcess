...
def draw_visualization_and_pub(vis, out_j, updater):    
    ...
    ## save right line position
    if (UTM_T_L) is not None and (updater.right_fit_wc is not None):
        right_f_wc = np.poly1d(updater.right_fit_wc)
        x_wc_to_draw_ = 6
        y_wc_to_draw_ = -right_f_wc(x_wc_to_draw_-updater.z_for_keep_distance)
        L_p_LP = np.array([x_wc_to_draw_,y_wc_to_draw_,0,1])
        UTM_p_UTMP = np.matmul(UTM_T_L,L_p_LP)
        saved_right_line_list.append((UTM_p_UTMP[0], UTM_p_UTMP[1]))
        print('LUTM', UTM_p_UTMP)
...
def callback_save_points(msg):
    import pandas as pd
    saved_right_line_arr = np.array(saved_right_line_list)
    df = pd.DataFrame(saved_right_line_arr)
    df.to_csv('saved_csv.csv')



UTM_T_L = None
saved_right_line_list = []
def callback_transform_rslidar(msg):
    global UTM_T_L
    UTM_T_L = np.array(msg.data).reshape(4,4)
...
if __name__ == "__main__":    
    ...
    rospy.Subscriber('/Save_points', Float32, callback_save_points)
    rospy.Subscriber('/transforms/LUTM_T_L',Float32MultiArray, callback_transform_rslidar)
