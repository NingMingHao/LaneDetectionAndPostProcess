...
'''    
    ...
    Subscribed topics: /rslidar_points_BP_F
    Published topic: /rslidar_points_BP_F/transformed
...
#include <transform_publisher/calib_robosenseConfig.h>

// RPY
float roll_alpha=-180*M_PI/180;
float pitch_beta=-3.6*M_PI/180;
float yaw_gamma=-54*M_PI/180;

// Translations
float translation_x=-0.1;
float translation_y=-0.8;
float translation_z=1.25;
...
// L_T_BPF: L_R_BPF and L_p_LBPF

// // ===== NEW Transform - Oct26 ==== //
// // RPY Angles
// float roll_alpha=-160*M_PI/180;
// float pitch_beta=0*M_PI/180;
// float yaw_gamma=-55*M_PI/180;

// // Translations
// float translation_x=-0.1;
// float translation_y=-0.75;
// float translation_z=1.17;
...
void callback_pointcloud(const boost::shared_ptr<const sensor_msgs::PointCloud2>& raw_cloud){
    ...
    transformed_cloud_msg.header.frame_id = "rslidar_front";
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("/rslidar_points_BP_F", 1, callback_pointcloud);
    pub_transformed_cloud = nh.advertise<sensor_msgs::PointCloud2>("/rslidar_points_BP_F/transformed", 1);
    ros::spin();
